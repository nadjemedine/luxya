...
export default function HomePage({ onNavigate }: HomePageProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [b, f, n, c] = await Promise.all([
        ...
        setBanners(b); setFeatured(f); setNewProducts(n); setCategories(c);
  ...
  return (
    <div>
      {/* Hero Banner */}
      {heroBanner ? (
        <div className="hero-banner" onClick={() => heroBanner.link && onNavigate('boutique')}>
          {heroBanner.image && (
            <img className="hero-banner-img" src={urlFor(heroBanner.image).width(800).url()} alt="" />
          )}
        ...
        <div className="hero-banner" onClick={() => onNavigate('boutique')}>
      ...
      {categories.length > 0 && (
        <div style={{ paddingTop: '20px' }}>
          <div className="container">
            <div className="section-title" style={{ marginBottom: '14px', textAlign: 'left' }}>
              {t('sections.categories')}
            </div>
          </div>
          <div className="categories-scroll">
            {categories.map(cat => (
              <div key={cat._id} className="category-pill" onClick={() => onNavigate('boutique', { categoryId: cat._id })}>
